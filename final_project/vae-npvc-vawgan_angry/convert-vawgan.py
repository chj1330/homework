...
tf.app.flags.DEFINE_string('corpus_name', 'savee', 'Corpus name')
tf.app.flags.DEFINE_string('checkpoint', None, 'root of log dir')
tf.app.flags.DEFINE_string('src', 'SNeu', 'source speaker [SF1 - SM2]')
tf.app.flags.DEFINE_string('trg', 'THap', 'target speaker [SF1 - TM3]')
...
tf.app.flags.DEFINE_string('file_pattern', './dataset/savee/bin/Test Set/{}/*.bin', 'file pattern')
...
def make_output_sp_name(output_dir, filename):
    basename = str(filename, 'utf8')
    basename = os.path.split(basename)[-1]
    basename = os.path.splitext(basename)[0]
    return os.path.join(
        output_dir, 
        '{}-{}-{}.sp'.format(args.src, args.trg, basename)
    )
    ...
    with sv.managed_session() as sess:
        ...
        while True:
            try:
                ...
                oSPname = make_output_sp_name(output_dir, feat['filename'])
                print('\rProcessing {}'.format(oFilename), end='')
                sf.write(oFilename, y, FS)
                with open(oSPname, 'wb') as fp:
                    fp.write(sp)
