...
tf.app.flags.DEFINE_string('dir_to_wav', './dataset/savee/wav', 'Dir to *.wav')
tf.app.flags.DEFINE_string('dir_to_bin', './dataset/savee/bin', 'Dir to output *.bin')
tf.app.flags.DEFINE_integer('fs', 22050, 'Global sampling frequency')
tf.app.flags.DEFINE_float('f0_ceil', 700, 'Global f0 ceiling')
...
def read(
    ...
    with tf.device('cpu'):
        with tf.name_scope('InputSpectralFrame'):
            files = tf.gfile.Glob(file_pattern) # file list (same order as 'ls -U')
            filename_queue = tf.train.string_input_producer(files)


            reader = tf.FixedLengthRecordReader(record_bytes)
            dd, value = reader.read(filename_queue)
            ...
            if data_format == 'NCHW': # channel x height x width
                feature = tf.reshape(feature, [1, SP_DIM, 1])
            elif data_format == 'NHWC': #Tensorflow default, height x width x channel
...
def pw2wav(features, feat_dim=513, fs=22050):
