...
class Wrapper(object):
    ...
    def __init__(self, input_size, num_class, learn_rate):
        ...
        self.optimizer = torch.optim.SGD(self.model.parameters(), lr=learn_rate, weight_decay=1e-6, momentum=0.9,
                                    nesterov=True)
    ...
    def run_model(self, x, y, device=0, mode='train'):
        ...
        for batch in range(x.shape[0]):
            input = Variable(torch.from_numpy(x[batch].transpose((1, 0, 2))).type(torch.FloatTensor)) # seq_len, batch, chroma
            #input = Variable(torch.from_numpy(x[batch].transpose((0, 2, 1))).type(torch.FloatTensor))
    ...
    def export(self, export_dir, chroma, annotation, prediction, acc_test):
        if not os.path.exists(os.path.dirname(export_dir)):
            os.makedirs(os.path.dirname(export_dir))
        checkpoint_path = "model_{:09f}.pth".format(acc_test * 100)
        model_save_path = os.path.join(export_dir, checkpoint_path)
